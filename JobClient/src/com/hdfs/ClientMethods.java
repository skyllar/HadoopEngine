...
public class ClientMethods {
	...
	public int submitMapReduceTask(String args[]) {
		int jobId = -1;
	...
	private void verifyTaskCompletion(Integer jobId) {
		int status = 2;
		QueryJobStatus queryJobStatus = new QueryJobStatus(clientConf, jobId);
		while (status == 2) {
			try {
				Thread.sleep(clientConf.jobStatusTimerDelay);
				queryJobStatus.run();
				status = queryJobStatus.globalStatus;
