...
public class JobTrackerInterfaceImpl extends UnicastRemoteObject implements
		...
		try {
			...
			if (mapSlotsFree > 0) {
				if (!mapperQueue.isEmpty()) {
					...
					int firsBlockInQueue = mapJobIdToJobProgress.get(jobId).blocksLeftForMapping
							.removeFirst();
					int taskId = ++mapJobIdToJobProgress.get(jobId).lastMapperIdAssigned;
					...
					mapJobIdToJobProgress.get(jobId).isMapIdCompleteStatusRecieved
							.put(taskId, false);
			...
			if (reduceSlotsFree > 0) {
				if (!reducerQueue.isEmpty()) {
					...
					int taskId = ++mapJobIdToJobProgress.get(jobId).lastReducerIdAssigned;
					// int taskId = ++jTCnf.taskIdCounter;
					...
					reducerTaskInfoBuilder.setOutputFile(mapJobIdToJobProgress
							.get(jobId).outputFile);

					mapJobIdToJobProgress.get(jobId).numReduceTasksStarted++;

					mapJobIdToJobProgress.get(jobId).isReduceIdCompleteStatusRecieved
							.put(taskId, false);
	...
	private int updateAndcheckAllReduceTasksCompletedForJobId(
		...
		if (mapJobIdToJobProgress.get(jobId).isReduceIdCompleteStatusRecieved
				.get(taskId) == false) {
			mapJobIdToJobProgress.get(jobId).isReduceIdCompleteStatusRecieved
					.put(taskId, true);
	...
	private int updateAndcheckAllMapTasksCompletedForJobId(
		...
		if (mapJobIdToJobProgress.get(jobId).isMapIdCompleteStatusRecieved
				.get(taskId) == false) {
			mapJobIdToJobProgress.get(jobId).isMapIdCompleteStatusRecieved.put(
					taskId, true);
