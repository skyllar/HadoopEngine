...
public class HDFSMethods {
	...
	public HDFSMethods(JobTrackerConfiguration jobTrackerCon) {
		jTCnf = jobTrackerCon;
	}
	...
	public BlockLocationResponse openFileInHDFSandGetBlockLoctions(
		...
		try {
			...
			INameNode nameNodeSkelton = (INameNode) Naming.lookup("rmi://"
					+ jTCnf.nameNodeIP + ":" + jTCnf.nameNodePort + "/"
					+ jTCnf.nameNodeRefernce);
			byte[] returnedMessage = nameNodeSkelton
			...
			if (status == 0) {
				...
				blockLocationRequestBuilder.addAllBlockNums(openFileResponse
						.getBlockNumsList());
				nameNodeSkelton = (INameNode) Naming.lookup("rmi://"
						+ jTCnf.nameNodeIP + ":" + jTCnf.nameNodePort + "/"
						+ jTCnf.nameNodeRefernce);
				returnedMessage = nameNodeSkelton
